...
void AiquamPlusPlus::run() {
    ...
    std::unique_ptr<struct area_data[]> sendbuf;

    // Define the receiving buffer
    std::unique_ptr<struct area_data[]> recvbuf;
    ...
    for (size_t fileIdx = 0; fileIdx < ncInputs; ++fileIdx) {
        ...
        if (world_rank == 0) {
            // Get the number of areas to be processed for each process
            size_t areasPerProcess = nAreas / world_size;

            // Get the number of spare areas for the process with world_rank==0
            size_t spare = nAreas % world_size;

            // The process with world_rank==0 will calculate extra areas
            send_counts[0] = (int)((areasPerProcess + spare));

            // The process with world_rank==0 will process the first areas
            displs[0]=0;
            ...
            for (int i = 1; i < world_size; i++) {
                ...
                send_counts.get()[i] = (int)(areasPerProcess);

                // Set the displacement
                displs.get()[i]=send_counts.get()[0]+areasPerProcess*(i-1);
            ...
            for (int idx = 0; idx < nAreas; idx++) {
                ...
                area.addTimeSeriesValue(current_conc);
            }
        }
    }
